...
- 内部页签：
  ...
  - **分时缩略线与明细抽屉**：自选/持仓每行带当日分时缩略曲线，点击弹出右侧抽屉：固定信息头（名称/代码/现价/涨跌幅/所属行业板块及板块涨跌幅/今开/昨收/换手率/最高/最低/成交量/成交额/市盈率/总市值），图上按持仓流水标注 **B/S 买卖点**（悬停显示「数量 @ 价格 · 分组/持仓」；周/月/年 K 把同周期多笔归并为 `B2`/`S3`），下方以 tab 切换 **分时 / 五日 / 日K / 周K / 月K / 年K**
