...
- **上游连接抖动**：本机到东财主机的连接会**随机被立刻关闭**（实测瞬时失败率 20–75%，与响应大小、IPv4/IPv6 选择无关，表现为 `UND_ERR_SOCKET`，几十毫秒即返回）。因此所有上游请求改为「**3 轮 × 每主机 2 次**」重试 + 抖动退避（并发 worker 不同步重试），12 秒总截止时间兜底；实测分时接口的冷启动成功率从 3/12 提升到 **12/12**、板块接口从 1/3 提升到 **6/6**，且正常时首次尝试即成功（240–500ms），重试不增加常态延迟
