...
- 内部页签：
  ...
  - **分时缩略线与明细抽屉**：自选/持仓每行带当日分时缩略曲线，点击弹出右侧抽屉：固定信息头（名称/代码/现价/涨跌幅/所属行业板块及板块涨跌幅/今开/昨收/换手率/最高/最低/成交量/成交额/市盈率/总市值），图表为**三窗格专业图**：主图 + **成交量**（红绿柱、最大量标注）+ **MACD**（HIST 柱 + DIF/DEA 线，12/26/9，HIST=(DIF−DEA)×2），主图与子图均带**纵轴刻度线**（1/2/5×10ⁿ 步长）；K 线叠加 **MA5/10/30/60**（橙/蓝/紫/灰，附当前值图例，均线在全量序列上计算后再截取可视区间）；K 线下方为**日期宽度缩放窗**（双滑块缩放/平移 + 近1月/3月/6月/1年/全部 快捷键 + 当前区间与根数），图表内**滚轮可以光标为中心缩放**（最少 8 根）。分时主图另叠加**均价线**，五日图为 5 个交易日**连续拼接**（午休/隔夜已折叠，交易日之间画分隔线）。
